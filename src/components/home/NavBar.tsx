...
export function NavBar(theProps: {getAvatar: (size: number) => JSX.Element, userNavigation: { name: string; href: string; onClick: () => void; }[], props: Partial<AppShellProps>}) {
  const { getAvatar, userNavigation, props } = theProps;
  return <Disclosure as="nav" className="bg-black bg-opacity-5">
    {({ open }) => (
      <>
        ...
        {props.user && <Disclosure.Panel className="md:hidden">
